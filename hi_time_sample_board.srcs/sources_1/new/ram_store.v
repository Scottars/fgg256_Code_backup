...
idle:begin
    ...
    if(store_en) begin
        ...
        if(cnt==99) begin
           cnt<=8'd0;
           ts_flag<=0;//跨秒指示回零
		   reg_us<=0;//马上开始新一轮发送，缓存的us时标应该先归零，不然可能误判（上一轮最后us=1M-1,本轮第一个us=0，就会误判为跨秒）
        end
        else begin
           cnt<=cnt+1'b1;
        end
...
us_judge:begin
  ...
  if(cnt_us<reg_us)//本次us<上次us：该数据是跨秒数据
    begin
      ts_cnt<=cnt;//本秒数据个数ts_cnt就等于当前已存储数据个数cnt
